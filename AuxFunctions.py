...
def animator(data, ZorC, Dim, tf):
    ...
    if ZorC == 'Z':
        if Dim == '2D':
            ...
            ani.save('SGSolSeedsin2D.gif', writer = FFwriter, dpi = 100)
        elif Dim == '3D':
            ani = animation.FuncAnimation(fig, update, frames = Ndt, interval = tf)
            FFwriter = animation.FFMpegWriter(fps = 1000)
            ani.save('SGSolSeedsin3D.gif', writer = FFwriter, dpi = 100)
        else:
            print('Please specify the dimension of the animation!')
    elif ZorC == 'C':
        if Dim == '2D':
            ani = animation.FuncAnimation(fig, update, frames = Ndt, interval = tf)
            FFwriter = animation.FFMpegWriter(fps = 1000)
            ani.save('SGSolCentroidsin2D.gif', writer = FFwriter, dpi = 100)
        elif Dim == '3D':
            ani = animation.FuncAnimation(fig, update, frames = Ndt, interval = tf)
            FFwriter = animation.FFMpegWriter(fps = 1000)
            ani.save('SGSolCentroidsin3D.gif', writer = FFwriter, dpi = 100)
        else:
            print('Please specify the dimension of the animation!')
    else:
            print('Please specify if you want to animate the centroids or the seeds!')